...
读取 [`references/form-contract.md`](references/form-contract.md)。如果目标表单仍是占位值，停止并请用户先填写自己的飞书收集页和资源标识；不要猜测目标。配置完成后，按实时表单字段生成一条简洁反馈。标题只描述一个现象；问题描述按“通俗说明、预期、实际、证据、可能原因”组织。
...
4. dry-run 成功后，展示“问题说明”和最终反馈字段，明确询问一次：`确认提交这条反馈吗？`
...
确认页面保持简短，只展示最终会写入飞书的内容和目标表单。等待确认时输出：
...
不要输出完整原始日志、冗长 CLI JSON、内部推理过程或重复的确认说明。
