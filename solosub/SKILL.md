...
草稿表格必须逐行展示所有当前可见题目。选填项没有内容时仍保留该行，写“未提供”或“本次不上传”，让用户看到最终提交边界。附件行展示将上传的文件名、类型和大小。若问题类型为 `Bug`，没有复现步骤或截图/录屏时，将其标为“缺失（提交前必须补充）”，不进入 dry-run 和确认阶段。
...
确认页面保持简短，但必须包含实时表单当前可见的每一个字段，字段名和顺序不得改变。人类可读表格之后紧跟字段完全对应的 Agent 可读块。等待确认时输出：
