...
草稿表格必须逐行展示所有当前可见题目，但只保留“收集表字段、必填、草稿内容”三列。选填项没有内容时仍保留该行，写“未提供”或“本次不上传”，让用户看到最终提交边界；附件行展示将上传的文件名、类型和大小。表格后增加“如何修改草稿”：先写“你可以直接告诉我‘字段名 + 新内容’。”，再只为单选、多选或其他枚举题列出实时返回的全部合法选项，不提供修改示例。若选择值不在实时选项中，标为“无效（请从可选项中选择）”，不进入 dry-run 和确认阶段。截图/录屏和复现步骤以实时 `required` 为准；非必填项缺失时只提醒“未提供（建议补充）”，不得阻止确认或提交。
...
确认页面保持简短，但必须包含实时表单当前可见的每一个字段，字段名和顺序不得改变。选择题的全部合法选项放在表格后的“如何修改草稿”，不得只显示当前值。人类可读内容之后紧跟字段完全对应的 Agent 可读块，并在 `field_options` 中保留同组选项。等待确认时输出：
